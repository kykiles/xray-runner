...
| `ALLOW_INSECURE` | `false` | Уважать флаг `insecure` из подписки (отключает проверку TLS-сертификата) |
...
| `--server <индекс\|имя>` | Выбрать сервер без меню: 1-based индекс или имя/хост (точное или уникальное частичное совпадение) |
| `--last` | Повторить последний выбор подписки/сервера (хранится в `last_server.json`, 0600) |
| `--non-interactive` | Никогда не спрашивать; если выбор невозможен — ошибка |
| `--version` | Показать версию |

Коды возврата: `0` — успех/выход пользователем, `1` — ошибка выполнения, `2` — не удалось выбрать сервер/подписку (для скриптов и systemd).

Интерактивный TUI: `↑/↓` или `j/k` — курсор, `Enter` — выбрать, `/` — фильтр по имени/хосту, `b` — бенчмарк latency (сортировка по результату), `r` — обновить подписку, `Esc/←` — назад, `q`/`Ctrl+C` — выход.
...
- Требует root (Linux) / прав администратора (Windows) — проверяется на старте
- **Маршрутизация:** программа сама маршруты ОС не настраивает — default route
  через `xray-tun` и исключение (host-route) до VPN-сервера выставляет
  xray-core (tun-inbound с `autoRoute`, зависит от версии/форка ядра).
  Если после поднятия TUN нет связи, проверьте `ip route` — должен быть
  маршрут по умолчанию через `xray-tun` и отдельный маршрут до IP сервера
  через физический шлюз; при его отсутствии добавьте host-route вручную:
